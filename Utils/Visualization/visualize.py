            ...
            if 'goal' in d:
                self.patches.append(
                    Rectangle((d["goal"][0] - 0.25, d["goal"][1] - 0.25), 0.5, 0.5, facecolor=Colors[0], edgecolor='black',
                              alpha=0.5))
        for name, path in self.combined_schedule.items():
            color = 'gray' if 'obs' in name else Colors[0]
            start_x, start_y = path[0]['x'], path[0]['y']
            self.agents[name] = Circle((start_x, start_y), 0.3, facecolor=color, edgecolor='black')
            self.agents[name].original_face_color = color
            self.patches.append(self.agents[name])
            self.T = max(self.T, path[-1]["t"])
            label = name.replace('agent', '').replace('obs_', 'O')
            self.agent_names[name] = self.ax.text(start_x, start_y, label)
